...
tf.app.flags.DEFINE_string("ps_hosts", "172.18.0.6:9906", "Comma-separated list of hostname:port pairs")
tf.app.flags.DEFINE_string("worker_hosts1", "172.18.0.2:9903,172.18.0.3:9904",
                           "Comma-separated list of hostname:port pairs")
tf.app.flags.DEFINE_string("worker_hosts2", "172.18.0.4:9905,172.18.0.5:9906",
                           "Comma-separated list of hostname:port pairs")
tf.app.flags.DEFINE_string("job_name", "", "One of 'ps', 'worker1', 'worker2'")
...
def main(_):
    ...
    worker_hosts1 = FLAGS.worker_hosts1.split(",")
    worker_hosts2 = FLAGS.worker_hosts2.split(",")

    # Create a new cluster
    cluster = tf.train.ClusterSpec({"ps": ps_hosts, "worker1": worker_hosts1, "worker2": worker_hosts2})
    ...
    if FLAGS.job_name == "ps":
    ...
    elif FLAGS.job_name == "worker1" or "worker2":
        # Decide which device to assign task
        with tf.device("/job:worker1"):

            # with tf.device(tf.train.replica_device_setter(worker_device="/job:worker1/task:%d" % FLAGS.task_index,
            #                                               cluster=cluster)):
                # built half model
        ...
        with tf.device("/job:worker2"):

            # with tf.device(tf.train.replica_device_setter(worker_device="/job:worker2/task:%d" % FLAGS.task_index,
            #                                               cluster=cluster)):
                # an other half model
        ...
        with tf.train.MonitoredTrainingSession(master=server.target, is_chief=(FLAGS.job_name == "worker2"),
